...
class NeuralNetwork:
    ...
    def train(self, X, y, epochs, learning_rate, batch_size=2):
        # Training loop
...
if __name__ == "__main__":
    ...
    labels_file_path = "t10k-labels-idx1-ubyte-fashion"  
    images_file_path = "t10k-images-idx3-ubyte-fashion" 
    ...
    # Ask for an image ID and make a prediction
    while True:
        try:
            image_id = int(input("Enter an image ID (0 to 2500): "))
            assert 0 <= image_id < 10000
        except (ValueError, AssertionError):
            print("Invalid input. Please enter a number between 0 and 9999.")
        else:
            selected_image = X_test[image_id]
            prediction = nn.predict(selected_image.reshape(1, -1))[0]

            # Display the image and prediction
            plt.imshow(selected_image.reshape(28, 28), cmap='gray')
            plt.title(f"Predicted Label: {fashion_mnist_labels[prediction]}")
            plt.show()
